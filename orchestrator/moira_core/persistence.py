...
class ExportSink:
    ...
    def on_audit(self, rec: AuditRecord, sealed: dict[str, Any] | None = None) -> None: ...
    def close(self) -> None: ...
...
class CompositeStore:
    ...
    def save_audit(self, rec: AuditRecord) -> dict[str, Any]:
        # the primary seals (hash-chains) the record and returns the sealed body;
        # fan THAT to sinks so the git mirror carries identical tamper-evidence.
        sealed = self.primary.save_audit(rec)
        self._fan("on_audit", rec, sealed)
        return sealed
    ...
    def mark_job_running(self, job_id: str, worker_id: str) -> int:
        return self.primary.mark_job_running(job_id, worker_id)

    def heartbeat_job(self, job_id: str, worker_id: str, lease_seconds: int = 300) -> int:
        return self.primary.heartbeat_job(job_id, worker_id, lease_seconds)

    def complete_job(self, job_id: str, worker_id: str, status: str,
                     error: str | None = None) -> int:
        return self.primary.complete_job(job_id, worker_id, status, error)
